...
- **多项目并发**：多个项目同时无人值守运行（默认上限 2，可调 1-4，超限自动排队等位）——每项目独立的会话池/会议/任务流/成本账，互不串扰；仪表盘一键切换查看
- **随时对话**：会议室团队频道里直接跟团队说话，协调者即时回答进度与问题；修改要求立刻落成优先任务插队处理（任意项目都可以，不限当前查看的）
...
- **集成回归门**：任务合并进 main 后，系统在主仓再跑一遍全项目 `test_cmd`——"后合并任务破坏先验收任务"的钩子在此被当场抓住：失败自动重开该任务（新工作树基于当前 main）修复回归，连续两次失败才阻塞升级
...
**实测参考**（诚实数字）：分级模型后单次开发回合约 $1.1（全 opus 约 $2.5-4）。一个 5 任务的 CLI 小项目全链路实测 $35（含 2 次合并冲突返工和多轮质疑交锋；同复杂度全 opus 基线 $57）。一次通过的任务全链路（开发→审查→QA→挑刺→终审）约 $3-5。**返工是成本大头**——这正是 BA、质疑者和团队记忆存在的理由。基准一（单项目）：中小型功能（4 任务，含收敛环/设计环/全部质检门）生产默认档 **$37.93 / 44 分钟 / 1 次干预**，见 [docs/DOGFOOD-timeline-v1.md](docs/DOGFOOD-timeline-v1.md)。基准二（**双项目并发**）：两个小型内核（3+3 任务）同时无人值守，**18.3 分钟墙钟（全程重叠）/ 合计 $25.44 / 0 次干预**，一次通过 5/6，集成回归门生产首秀 6/6，见 [docs/DOGFOOD-dual-v0.3.md](docs/DOGFOOD-dual-v0.3.md)。
...
| `integration_gate` | `on` | 集成回归门：任务合并 main 后在主仓跑全项目 `test_cmd`，失败自动重开任务修回归 |
| `concurrency.<role>` | reviewer/qa `2`，其余 `1` | 角色任务阶段并发数（1-4，副本会话） |
| `max_concurrent_projects` | `2` | 同时运行的项目流上限（1-4）；超限启动的项目自动转暂停等位 |
| `auth_token` | 空 | 非空后所有 API/WS 要求 Bearer token（局域网共享前必须设置）；空 = 仅本机零摩擦 |
| `cors_origins` | 空 | CORS 额外白名单（逗号分隔完整 Origin）；本机回环恒放行 |
...
- Agent 的文件写入被限制在各自项目工作区内；Bash 命令经白名单策略过滤，`rm -rf`、`git push`、联网命令、装依赖等一律转审批；**用户 MCP 写类工具同样受工作区边界约束**（指向工作区外的绝对路径 / file:// 一律拒绝，硬规则不走审批）
- **局域网共享前设置访问 token**（设置→安全）：非空后所有 API 与 WebSocket 都要求 Bearer token，浏览器首次访问弹解锁框；CORS 默认只放行本机回环，跨设备 Origin 需加入白名单。默认（空 token）仅监听 127.0.0.1，本机零摩擦
