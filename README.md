...
**实测参考**（诚实数字）：分级模型后单次开发回合约 $1.1（全 opus 约 $2.5-4）。一个 5 任务的 CLI 小项目全链路实测 $35（含 2 次合并冲突返工和多轮质疑交锋；同复杂度全 opus 基线 $57）。一次通过的任务全链路（开发→审查→QA→挑刺→终审）约 $3-5。**返工是成本大头**——这正是 BA、质疑者和团队记忆存在的理由。最新基准：中小型功能（4 任务，含收敛环/设计环/全部质检门）生产默认档 **$37.93 / 44 分钟 / 1 次干预**，全过程数据见 [docs/DOGFOOD-timeline-v1.md](docs/DOGFOOD-timeline-v1.md)。
